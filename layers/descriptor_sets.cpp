    ...
    for (auto binding_req_pair : binding_req_map) {
        ...
        if (flags & (VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT_EXT | VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT_EXT)) {
            if (!(flags & VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT_EXT)) {
                cb_node->validate_descriptorsets_in_queuesubmit[set_][pipe->pipeline][binding].requirements = binding_req_pair.second;
                cb_node->validate_descriptorsets_in_queuesubmit[set_][pipe->pipeline][binding].cmd_type = cmd_type;
            }
            continue;
        }
